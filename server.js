...
var trainingImages = [];
try { if (fs.existsSync(TRAINING_FILE)) trainingImages = JSON.parse(fs.readFileSync(TRAINING_FILE, 'utf8')); } catch(e) { trainingImages = []; }
var sessionTrained = {};
...
function extractRam(code) {
  ...
  if (upper.startsWith('KM') && !upper.startsWith('KLM') && !upper.startsWith('KLU')) {
    ...
    if (main.length >= 3) { var ch = main[main.length-2]; if (SAMSUNG_RAM_MAP[ch]) return SAMSUNG_RAM_MAP[ch]; }
  }
...
function searchInDB(code) {
  ...
  var keys = Object.keys(NORMAL_DB);
  for (var i = 0; i < keys.length; i++) {
    ...
    for (var j = 0; j < codes.length; j++) {
      if (u === codes[j].toUpperCase() || u.startsWith(codes[j].toUpperCase().split('-')[0])) {
        ...
        var ram = parts[1] ? parts[1].replace(/D[0-9]|z|m|UMCP/g,'').trim() : extractRam(u);
        return { code: u, storage: parts[0], type: 'عادي', company: detectCompany(u), ram: ram };
      }
    }
  }
  var ekeys = Object.keys(EMMC_DB);
  for (var i = 0; i < ekeys.length; i++) {
    var size = ekeys[i];
    var codes = EMMC_DB[size];
    for (var j = 0; j < codes.length; j++) {
      if (u === codes[j].toUpperCase() || u.startsWith(codes[j].toUpperCase().split('-')[0])) {
        return { code: u, storage: size, type: 'زجاجي', company: detectCompany(u) };
      }
    }
  }
  var mkeys = Object.keys(MICRON_DB);
  for (var i = 0; i < mkeys.length; i++) {
    var size = mkeys[i];
    var codes = MICRON_DB[size];
    for (var j = 0; j < codes.length; j++) {
      if (u === codes[j].toUpperCase()) return { code: u, storage: size, type: 'زجاجي', company: 'Micron' };
    }
  }
  return null;
}
...
// Gemini Prompt
var GEMINI_PROMPT = 'أنت خبير في شرائح الذاكرة (Memory IC chips). بتتكلم مصري.\n'
...
+ '=== عادي (Normal BGA) ===\n'
+ 'Company: Samsung - Code prefix: KM\nStorage: the letter BEFORE 100/200/600/700/800/900 -> N=8G | E=16G | X or D=32G | C or H or P=64G | G or V=128G | F or S=256G\n'
+ 'RAM: S or 2=1GB | 6=1.5GB | K or 1 or 3=2GB | A or B or 8=3GB | D or 4=4GB | C or J=6-8GB | P=8GB | L=10GB | M=12GB\n'
+ 'Company: SK Hynix - Code prefix: H9\nStorage: digits after first 4 chars -> 17/18/19=16G | 26/27=32G | 52/53=64G | 16=128G\n'
+ 'RAM: A4=0.5GB | A8=1GB | AB=2GB | AD=3GB | AC=4GB | AE=6GB\n'
+ 'Company: Kingston - TAIWAN - Storage written explicitly\n'
+ 'Company: SanDisk - SDIN - Storage written explicitly\n'
+ '=== زجاجي (eMMC/UFS) ===\n'
+ 'Samsung KLM/KLU: 5th char -> AG=16G | BG=32G | CG=64G | DG=128G | EG=256G | FG=512G\n'
+ 'SK Hynix H26/H28/HN8: 54=16G | 64=32G | 74=64G | 88=128G | 9=256G\n'
+ 'Toshiba THG: G7=16G | G8=32G | G9=64G | T0=128G | T1=256G | T2=512G\n'
+ 'SanDisk SDIN: Storage written explicitly\n'
+ 'Micron JW/JZ: full code lookup from table\n'
+ 'YMEC: YMEC6=32G | YMEC7=64G | YMEC8=128G | YMEC9=256G\n'
+ 'UNIC 08EMCP/16EMCP: 05G=32G | 06G=64G | 07G=128G\n'
+ 'Kingston زجاجي: EMMC - Storage explicit\n'
  ...
  try {
    ...
    if (isFirstCall && trainingImages.length > 0) {
      ...
      for (var i = 0; i < trainingImages.length; i++) {
        parts.push({ inlineData: { mimeType: 'image/jpeg', data: trainingImages[i].b64 } });
        if (trainingImages[i].label) parts.push({ text: trainingImages[i].label });
      }
    ...
    var gResult = await model.generateContent({ contents: [{ parts: parts }], generationConfig: { temperature: 0 } });
    var text = gResult.response.text().replace(/```json/g, '').replace(/```/g, '').trim();
    console.log('Gemini:', text);

    var parsed = null;
    try { parsed = JSON.parse(text); } catch(e) { var m = text.match(/\{[\s\S]*?"code"[\s\S]*?\}/); parsed = m ? JSON.parse(m[0]) : null; }
    ...
    if (dbResult) {
      return res.json({ code: parsed.code, storage: dbResult.storage, type: dbResult.type, company: dbResult.company, ram: dbResult.ram || extractRam(parsed.code) || parsed.ram, step: 'gemini+db', confidence: 95 });
    }

    return res.json({ code: parsed.code, storage: parsed.storage || '', type: parsed.type || '', company: parsed.company || detectCompany(parsed.code), ram: parsed.ram || extractRam(parsed.code), step: 'gemini', confidence: parsed.storage ? 80 : 40 });
  } catch(error) {
    console.error('Analyze error:', error.message);
    var msg = (error.message || '').toLowerCase();
    var errMsg = 'خطأ في التحليل';
...
app.post('/api/chat', async function(req, res) {
  try {
    ...
    if (history && history.length > 0) {
      historyText = '\nتاريخ:\n';
      history.slice(-10).forEach(function(h) { historyText += (h.role === 'user' ? 'المستخدم' : 'أنت') + ': ' + h.text + '\n'; });
    }
    var chatPrompt = 'أنت خبير في شرائح الذاكرة (Memory IC chips). بتتكلم مصري.\n'
      + 'النوع: عادي أو زجاجي\n'
      + (context ? 'آخر كود: ' + (context.code||'-') + ' | ' + (context.storage||'?') + 'GB ' + (context.type||'?') + ' | ' + (context.company||'?') + ' | رام: ' + (context.ram||'?') : '')
      + historyText + '\n'
      + 'رسالة: "' + message + '"\n'
    ...
    var gResult = await model.generateContent({ contents: [{ parts: parts }], generationConfig: { temperature: 0.7, maxOutputTokens: 500 } });
...
app.post('/api/confirm', function(req, res) { if (req.body.code) console.log('تأكيد:', req.body.code); res.json({ success: true }); });
app.post('/api/correct', function(req, res) { if (req.body.code) console.log('تصحيح:', req.body.code); res.json({ success: true }); });
app.get('/api/cache-count', function(req, res) { res.json({ count: 0 }); });

app.get('/api/training', function(req, res) {
  res.json({ count: trainingImages.length, images: trainingImages.map(function(t,i) { return { index: i, label: t.label || '', hasImage: !!t.b64 }; }) });
});
app.post('/api/training/add', function(req, res) {
  ...
  trainingImages.push({ b64: b64, label: label || 'مثال ' + (trainingImages.length + 1) });
